...
import os
input = open(os.path.abspath('../input/rosalind_ba2d.txt'))
...
def buildProfile(motifs, size):
    ...
    for i in range(k):
        ...
        for motif in motifs:
            ...
            else: posProfile[motif[i]] = 1
        for base in bases:
            if base not in posProfile: posProfile[base] = 0
            if base not in profile: profile[base] = [float(posProfile[base]) / size]
            else: profile[base].append(float(posProfile[base]) / size)
