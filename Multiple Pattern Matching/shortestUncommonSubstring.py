import os
import sys
input = open(os.path.abspath('../input/rosalind_ba7f.txt'))
if not os.path.exists('../output/'): os.mkdir('../output/')
output = open(os.path.abspath('../output/rosalind_ba7f.txt'), 'w')
...
allPatterns = []
for node in color:
    if color[node][0] == 'blue': blueNodes.append(node)

for node in blueNodes:
    pattern = backtrack(0,node)
    if '$' not in pattern: allPatterns.append(pattern)
    
print min(allPatterns, key=len)
output.close()
